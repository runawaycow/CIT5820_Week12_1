...
def fill_order(order,txes=[]):
    for existing_order in g.session.query(Order).filter(Order.filled==None).filter(Order.buy_currency==order['sell_currency']).filter(Order.sell_currency==order['buy_currency']).all():
        if order['sell_amount'] * existing_order['sell_amount'] >= order['buy_amount'] * existing_order['buy_amount']:
            order_obj = Order(filled=datetime.now(), counterparty_id=existing_order.id, **order)
